...
"@
        ...
        # Read as UTF-8 (BOM-aware) instead of Get-Content, which decodes a
        # BOM-less UTF-8 file with the host's ANSI codepage under Windows
        # PowerShell 5.1 and therefore over-counts every non-ASCII character.
        $chars = ([System.IO.File]::ReadAllText($Path)).Length
...
# The python3/tiktoken probe above leaves $LASTEXITCODE set to the probe's
# failure code (9009 when python3 is absent), which would otherwise become this
# script's exit code and make a successful count look like a failure to callers.
exit 0

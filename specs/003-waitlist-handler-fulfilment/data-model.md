...
| `AcceptCommand` / `CompleteCommand` / `ReleaseCommand` / `CancelCommand` | `ICommand?` | The view model's commands, carried to the card through `Order`. Null when the flag is false. |
| `ReleaseCommand` | `ICommand?` | The Give back action. It was **bound by nothing** until 2026-09-13 — the card's action area held two buttons — and is now drawn beside Complete under the same `CanCompleteOrRelease` gate (**superseding** that decision; FR-047 in `specs/004-unified-card-item-picker`, and §C7 of this feature's `action-contracts.md`). |
| `AcceptActionText`, `CompleteActionText`, `ReleaseActionText`, `CancelActionText` | `string` | Localized automation names **and tooltips** for the icon buttons — accessibility without a literal in markup. |

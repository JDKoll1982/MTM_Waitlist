...
- [ ] **T050** [P] [US2] Build the create page as a form only with no actions area, its fields localised, no fixed width, and its role picker reading the one catalogue · `Module_Settings/Views/CreateUserPage.xaml`, `Module_Settings/Views/CreateUserPage.xaml.cs` · FR-099, FR-113, FR-116
...
- [ ] **T067** [P] [US8] Build the badge lookup keyed on the role code with one entry per role the catalogue holds, a fallback reachable only by a genuinely unknown or blank role, and the branches naming roles the catalogue has never held — `administrator`, `supervisor`, `manager`, `quality`, `quality inspector` and `Setup Tech` — removed rather than carried forward · `MTM_Waitlist.Core/Permissions/RoleBadgeCatalog.cs` · FR-105, FR-106, FR-107
- [ ] **T068** [P] [US8] Point the shell's badge at the lookup and at the signed-in person's role code, so every role the catalogue holds gets its own badge instead of the grey default, and the badge stays presentation rather than becoming a permission · `ViewModels/ShellViewModel.cs` · FR-058, FR-105

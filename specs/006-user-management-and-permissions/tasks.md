...
- [x] **T056** [P] [US5] Add the page's models and its view model: one cell per person and permission carrying the permission's declared label, its value, its provenance and its pending state, and a change set carrying both values per entry so the store can refuse a value that has moved; then the view model itself — **the matrix rebuilt as one card per area, the area's permissions as the card's columns and the people as the card's rows**, built from the declaration so no permission and no area can be lost, with the three cell states carried by shape rather than by colour and named in words, a person who outranks the reader shown with every cell of their row unavailable and the reason in words, the fixed cell present and locked with its reason and clearable by nobody, pending changes marked until saved, saving with nothing changed unavailable and writing no history record, one save per person because the change-set procedure takes one person, a confirmation naming in the reader's words what changes and for whom with one sentence per changed cell and a count when several change, an undo that reverses the whole save rather than half of it and that names the permission whose value has moved and asks before restoring, and a warning stating how many changes are pending before the page is left · `MTM_Waitlist.Settings/Models/PermissionCell.cs`, `MTM_Waitlist.Settings/Models/PermissionAreaCard.cs`, `MTM_Waitlist.Settings/Models/PermissionCardRow.cs`, `MTM_Waitlist.Settings/Models/PermissionColumn.cs`, `MTM_Waitlist.Settings/Models/PermissionMatrixRow.cs`, `MTM_Waitlist.Settings/Models/PermissionChangeSet.cs`, `MTM_Waitlist.Settings/ViewModels/PermissionsViewModel.cs` · FR-059, FR-063 to FR-074, SC-010
- [x] **T057** [P] [US5] Build the permissions page — the five area cards with the area's name at the left of each heading row and its permissions across the top, the people as the rows, the three cell states drawn as three shapes with an accessible name for each, the fixed cell rendered locked with its reason, no control with a fixed width and a card that scrolls sideways rather than dropping a column, the unsaved block naming the people with pending changes, and Back returning to Settings without walking back through repeats of the page — and register it through the partial method declared in T021, in this phase's own registration file · `Module_Settings/Views/PermissionsPage.xaml`, `Module_Settings/Views/PermissionsPage.xaml.cs`, `Services/DependencyInjection/ServiceRegistrationExtensions.Permissions.cs` · FR-063, FR-080, FR-083, FR-113, FR-116, constitution V
...
- [x] **T059** [P] [US5] Assert the page's behaviour: the cards are the declaration's five areas in their order and every declared permission is in exactly one of them, the rows are the people in rung-then-name order, the three cell states are three different shapes with three different accessible names, a changed cell is pending with the count and the listing that follow from it, the confirmation's wording and its count, saving with nothing changed unavailable, one save per person writing that person's whole set, the undo reversing the whole save for the person it was written for, the moved-value prompt before restoring, the outranking person's whole row shown unavailable with the reason in words, the fixed cell locked and clearable by nobody, the unsaved-change warning stating its count, and one impatient press of save writing exactly one change, with no screen in this feature freezing or refusing interaction while it loads or saves · `MTM_Waitlist.Tests/Module_Settings/ViewModels/PermissionsViewModelTests.cs` · FR-066, FR-067, FR-068, FR-069, FR-070, FR-073, FR-074, FR-114, SC-010

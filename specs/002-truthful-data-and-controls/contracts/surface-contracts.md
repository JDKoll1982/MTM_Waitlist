...
- Section behaviour follows the block-state model: no rows with a successful empty read ⇒ the section is
  ...
  error text plus a retry action **in place**, with the rest of the page unaffected. The rule is stated once,
  in spec FR-002 — this is its surface form, not a second source.
...
<!-- token-budget: compacted (level=medium) on 2026-09-12T23:51:50Z; original at surface-contracts.full.md -->

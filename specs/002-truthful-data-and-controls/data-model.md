...
This feature persists nothing, so there is no schema. What follows models *what the surfaces must express* and the *inventory the change operates on* — the reference the contracts, the tasks and the verification are checked against.
...
1. `Failed` is never rendered as `Hidden` or `Row absent` (no failure disguised as absence).
2. `Hidden` is never accompanied by a placeholder value, a stand-in figure, or an empty labelled shell.
3. A page-level **store-outage** state keeps its existing meaning (`specs/001` FR-021: message, retry, no sample data); a block failure never renders as it, and it is never reused for one.
...
Model: `SettingsViewModel` holds a declared list of the property names that consume `MatchesSearch(...)`, and `RefreshSearchVisibility()` raises `PropertyChanged` for every name in it.
...
<!-- token-budget: compacted (level=medium) on 2026-09-12T23:51:49Z; original at data-model.full.md -->

...
- **FR-024**: The system MUST prefer live external data whenever the external source is reachable and MUST serve from the cached copy only while the source is unreachable. "Unreachable" MAY be taken from the reachability probe's settled verdict rather than re-established on every read, in which case the cached copy is served while a verdict of unreachable is in effect and recovery MUST be honoured by the very next read after the probe reports the source reachable again (so the worst case is one probe interval of staleness, bounded by `CachedProbeInterval`). A reachable source that legitimately returns no rows MUST NOT be replaced by cached data.

...
- **Analyze remediation (2026-09-09)**: all `/speckit.analyze` findings were fixed — `FR-021` clarified (bounded backoff 3×≈1/2/4 s + named surface), `FR-023` de-duplicated against `FR-010`, the mirror proc-name drift corrected (`quickstart.md`/`data-model.md` → `sp_visual_<shapeKey>_{get,refresh}`), the status model unified (`ReadStatusSnapshot` + `VisualReadStatus` enum), plan↔tasks phase numbering mapped (`tasks.md` §Phase numbering), and tasks `T107`–`T111` added (FR-021 handling + verification, the `sp_visual_read_shape_metadata_get` metadata procedure, and the FR-020 no-downtime check).

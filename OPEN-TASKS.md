...
| ~~T164~~ | ~~MEDIUM~~ | **RESOLVED 2026-09-12.** An account still holding the temporary default password had to be signed into with `0000` before the change panel appeared. Startup now resolves the requirement from the store while the splash is up (new `sp_auth_password_reset_required_get`, called only on the login branch) and the window opens directly on the change panel — the sign-in form is never shown. The auto-login path is deliberately unchanged. See `tasks.md` Phase 32. **Superseded 2026-09-21:** opening straight on the change panel skipped the credential check entirely, so anyone at that workstation could set the account's password and the attempt limit could never apply to a credential nobody was asked for. It was defensible while the temporary value was the shared `0000` constant, which proves nothing by being typed; decision 9 replaced it with a per-person PIN, and the panel now opens only once that credential has been accepted at sign-in. |

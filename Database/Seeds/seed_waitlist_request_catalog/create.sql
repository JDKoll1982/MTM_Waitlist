...
-- Canonical mapping (2026-09-08): leaf rows only. Types that group
-- subtypes (Pickup, Coil, Scrap, ...) are NOT directly selectable, so
-- category/item_id stay NULL. Only Forklift Assist (0 subtypes) is a leaf.
-- ---------------------------------------------------------------
INSERT INTO waitlist_request_types
    (public_id, request_type, control, flow, requires_text_input, prompt_text,
     min_length, max_length, default_image_path, category, item_id,
...
VALUES
    ('7bb056da-2dfd-4da5-824c-cff0973544fb', 'Pickup',
     ...
     'direct-to-confirmation', 0, NULL, 0, 200, NULL, NULL, NULL,
...
-- Canonical mapping (2026-09-08): every subtype is a leaf the wizard can
-- submit, so each carries its canonical category + item_id (23-row spec).
-- Mapping per Plan-Design-TypeCategoryActionRefactor clarifications:
--   Pickup Other/General Text Entry -> Other/other
--   Pickup NCM/WIP/FG -> Pickup/pickup-ncm|wip|fg
--   Pickup Coil/Flatstock, Coil Pickup, Flatstock Pickup -> Pickup/pickup-coil (merged)
--   Coil Bring -> Deliver/deliver-coil ; Flatstock Bring -> Deliver/deliver-flatstock
--   Coil Wrong Coil @ press -> Deliver/deliver-wrong-coil
--   Coil Need Riser Table -> Deliver/deliver-riser-table
--   Coil Need Coil Turned around -> Assist/assist-coil-turn
--   Scrap Empty -> Pickup/pickup-scrap ; Pickup Hopper do-not-return -> Pickup/pickup-hopper
--   Scrap Bring Hopper -> Deliver/deliver-hopper
--   Flatstock Wrong Flatstock -> Deliver/deliver-wrong-flatstock
--   Table Place/Remove Parts -> Assist/assist-table-place|remove
--   Die Bring Die -> Deliver/deliver-die ; Pull Die variants -> Pickup/pickup-die
-- ---------------------------------------------------------------
INSERT INTO waitlist_request_subtypes
    (public_id, request_type_id, subtype_name, control, flow, requires_text_input, prompt_text,
     min_length, max_length, default_image_path, category, item_id, center_data_grid_fields_json,
     is_active, created_utc, updated_utc)
SELECT s.public_id, rt.id, s.subtype_name, s.control, s.flow, s.requires_text_input, s.prompt_text,
       s.min_length, s.max_length, s.default_image_path, s.category, s.item_id, s.center_data_grid_fields_json,
...
FROM (
    SELECT 'feaa6b5e-13db-4ad8-9c17-188f40ca41ed' AS public_id, '7bb056da-2dfd-4da5-824c-cff0973544fb' AS parent_public_id,
           ...
           'Other' AS category, 'other' AS item_id,
    ...
    UNION ALL SELECT '4d8f6d3b-6116-4d07-bd4a-3cc420b8001f','805c5b0f-815f-46bf-b5a7-73de5d74fa1f','Wrong Flatstock @ Workcenter',
           'MTM_Waitlist.Module_Waitlist.Controls.Flatstock.FlatstockRequestTypeImageView','direct-to-confirmation',0,NULL,0,200,NULL,
           'Deliver','deliver-wrong-flatstock',
           JSON_ARRAY('Part','Quantity','Work center','Destination'),1

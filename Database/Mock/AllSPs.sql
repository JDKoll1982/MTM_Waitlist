...
-- deliberately NOT applied here (data-model.md Â§3.4) â€” the mirror returns the same rows the live
...
-- for the full rationale): truncate stage â†’ load complete payload â†’ validate â†’ atomic 3-name swap.
--
--   p_rows : JSON array of result objects. Keys:
...
-- ============================================================
-- Artifact: sp_visual_read_shape_freshness_get
-- ============================================================
...
-- baseline seed rows (FR-017, FR-022, data-model.md Â§12).
--
...
-- branch below. That is additive by construction â€” the output columns are fixed, so no existing
...
-- parameter name and meaning as the live read (data-model.md Â§3.3). `PartNumber` in the RETURNED
...
-- what it means in the live result (data-model.md Â§3.3).
-- ============================================================
...
-- complete previous snapshot or the complete new one â€” never a partial set and never an empty
-- table (FR-006, SC-005). See research.md R1 and data-model.md Â§3.6.
--
-- Contract (design A â€” confirmed 2026-09-09)
-- ------------------------------------------
...
--            (an empty live read is a valid answer â€” FR-024).
--
-- Steps
-- -----
...
-- (data-model.md Â§11: mtm_mock carries no table other than the mirrors and their stage twins).
-- ============================================================
...
BEGIN
    ...
    -- 4. Atomic swap â€” the live name always resolves to a complete snapshot.

...
| **Status** | **FIXED — closed by `specs/002-truthful-data-and-controls`**. **Closure:** the inert Cancel and Accept buttons were removed from the card, and the request's real lifecycle status is shown in the action area instead — no control is offered whose activation has no effect. **Remainder owned by Spec `02-handler-fulfilment-and-urgency`**, where the actions return wired to the commands and gates in §5 of this file. **Proof:** `MTM_Waitlist.Tests/Module_Waitlist/Controls/WaitlistLineCardMarkupTests` (no `Button` without a `Command`; the Cancel and Accept elements are gone). |

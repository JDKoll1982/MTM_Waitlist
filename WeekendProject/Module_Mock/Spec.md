...
| 5 | **In-app fallback.** Add the `IVisualReadFallback<TRequest,TRow>` implementation, route the originating caller through it, and register it in DI. No existing shape's contract changes. The shared `VisualReadFallback<,>` base supplies the settled-verdict short-circuit (contract §2a), so a new shape inherits it by passing the reachability detector through its constructor. | `MTM_Waitlist.Mock/Services/Visual<Shape>Fallback.cs` (+ DI registration) |

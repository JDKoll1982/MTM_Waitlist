...
> Deliver 6 / Assist 2 / Other 1 — **expanded 2026-09-08 to 23 rows**: Pickup 11 / Deliver 8 / Assist 3 /
> Other 1). Aligns with card/fulfill Phase 2 (file `07`); consumes the design docs
...
> **Status (2026-09-09):** Phase 0, 1.1 (live-green), 1.2 #1/#2, disposition groundwork, Phase 2
> resolvers/SPs, Phase 3 data/flow core, Phase 5 NCM defect backend, and the **Phase 7 CI/CD
> build/suite gate** are landed — **27/47 = 57%**. The open boxes are now the **interactive in-app
> UI phases**: Phase 3 picker (Category→Item re-layout), Phase 4 uniform 2-line card, Phase 5 Frontend
> defect panel + NCM Line-2 wiring, Phase 6.2 real FG/WIP/Outside item wiring, and Phase 7
> QA/security/cleanup. Those require running the WinUI app (+ XamlMcp) to build and visually verify;
> the "Next implementation process" preamble below scopes that session.

---

## NEXT IMPLEMENTATION PROCESS (interactive in-app session — Phases 3/4/5F/6.2/7)

> Everything below this box-set that is still `- [ ]` is bound by the CSV/design specs and by the
> already-landed data core (Phase 2 resolvers + Phase 3 picker rules + Phase 5 backend + Phase 6
> groundwork), but each remaining box flips a **live UI surface** that must be built in the running
> WinUI app and verified with XamlMcp before it can be ticked.
> Execution order for that session: **Phase 3 (picker UI)** → **Phase 4 (uniform card)** → **Phase 5
> Frontend (defect panel + NCM Line 2)** → **Phase 6.2 finish** → **Phase 7 (tests/cleanup/security)**.
> Composition-root note (carried from CONTINUE-IMPLEMENTATION.md): `MTM_Waitlist.Waitlist.NewRequest`
> references Settings (catalog/picker rules) but NOT Setup — introduce a composition-root bridge (app
> service or contract move) to reach `IActiveJobItemResolverService` (Setup) before wiring Phase 3
> conditional visibility / Phase 4 render-time item data / Phase 6.2 real part-sequence.
> First unchecked task: **Phase 3 — Request Type Card: re-lay New Request category selection**
> (Category Pickup/Deliver/Assist/Other then Item) | **Persona: Frontend Engineer**.
...
> **Data/flow core DONE 2026-09-09** (see Phase 3 notes in CONTINUE-IMPLEMENTATION.md): the rules
> (`RequestItemPickerRules` + `RequestJobPartAvailability`, 9 tests) and option assembly
> (`NewRequestPickerService` + `INewRequestPickerService`, 4 tests) are green and DI-registered.
> **INCREMENT #1 LANDED 2026-09-09 (build 0w/0e; full suite 622 total / 0 failed / 609 passed / 13 skipped):**
> canonical Category→Item re-lay of the Job Type step — `NewRequestCanonicalPicker` (pure grouping of the
> DB leaf tree into canonical Category/Item groups with deterministic primary-leaf resolution, CSV order)
> + `NewRequestCanonicalCategory/Item` + `NewRequestPickerTile` models + re-laid `NewRequestJobTypeViewModel`
> (two-stage Categories→Items with legacy DB-down fallback, coil banner, Back-to-categories) + `NewRequestJobTypePage`
> bound to the new `Options` surface. 9 `NewRequestCanonicalPickerTests` + reworked Job-Type VM tests green.
> NOT yet ticked: needs in-app visual verification (XamlMcp). Remaining Phase 3 boxes: per-part conditional
> visibility (Setup resolver bridge), dunnage/component/outside-service items (no DB leaf yet — data-source
> increments), user-entry item UIs, zero-payload flags.
> Next task: **Request Type Card: re-lay New Request category selection to Category then Item.** | **Persona: Frontend Engineer**

- [ ] **Request Type Card: re-lay New Request category selection** to Category (Pickup/Deliver/Assist/Other) then Item, matching CSV Order. (Ref: CSV Order col) | **Persona: Frontend Engineer** — *increment #1 (2026-09-09): canonical two-stage picker landed (see Phase 3 note above) — categories in canonical order, items in CSV order, selecting an item submits its resolved legacy leaf (DB RequestType+Subtype) so the downstream steps are unchanged; DB-down legacy fallback preserved; items with no DB leaf yet (component/outside-service/dunnage) surface in later increments. TICK after in-app (XamlMcp) visual verification.*
...
> Also lands Phase 1.2 #3 (`ResolveImagePath` → Category/Item) and Phase 1.2 #4
> (`GetDefaultTypes()` order by Category then Item) which were deferred here by user scope.
> Next task: **Request Type Card: build single uniform 2-line card control.** | **Persona: Frontend Engineer**
...
- [x] **Settings Page: gate the defect editor** to Admin/Developer roles. (Ref: repo role gating pattern) | **Persona: Backend Engineer** — verified 2026-09-09 (service-layer enforcement): new `MTM_Waitlist.Settings/Services/DefectTypeCatalogService.cs` gates Add/Update/Delete behind `CanManage(role)` (Admin/Developer/Administrator), mirroring the Setup role-param pattern; 9/9 `DefectTypeCatalogServiceTests` green (role allowed/denied + no SP call on denial). The Module_Settings panel (box #3) surfaces/consumes it.
...
- [ ] **Testing: add/adjust unit tests** for catalog load, resolvers, New Request picker, and card render across all 18 CSV rows (now 23 canonical rows). (Ref: CSV; existing `SampleWaitlistRequestCatalog`) | **Persona: QA Engineer** — *catalog-load/resolver/picker-rules portions already green (`RequestItemCatalogTests`, `ActiveJobItemResolverServiceTests` 8, `RequestItemPickerRulesTests` 9, `NewRequestPickerServiceTests` 4); the card-render + end-to-end picker portions land with Phases 3/4 and are added here when those UI surfaces exist.*
...
- [x] **CI/CD: full solution build + full test suite green** with 0 warnings. | **Persona: DevOps Engineer** — verified 2026-09-09: `dotnet build MTM_Waitlist.sln -c Debug -p:Platform=x64 /m:1 /nodeReuse:false` → `Build succeeded. 0 Warning(s) 0 Error(s)`; `dotnet test MTM_Waitlist.Tests/MTM_Waitlist.Tests.csproj -c Debug -p:Platform=x64 /m:1` → `total: 608, failed: 0, succeeded: 595, skipped: 13` (13 opt-in MySQL DB-integration tests skipped offline; the DB-integration subset passes live on localhost). Re-run as the final gate after Phases 3/4/5F/6.2 land.

...
- [x] **Workflow: Setup screen location lists (`SetupSubordinatePart` grids) omit ignored locations.** *(Ref: Master Task 12)* | **Persona: Frontend Engineer** *(2026-09-07: `SetupLookupService` now injects `IIgnoredLocationsService` and filters subordinate parts whose `Location` is ignored, covering mock + backend paths)* — **SUPERSEDED 2026-09-24: the Setup process no longer consults the ignored-locations set.** A coil's requirement location in Infor Visual *is* the plant or work-centre code it was issued to, so the filter removed whole parts from the job: WO-074171's coil (location `WC`) vanished from the Setup review, from the saved record and therefore from the waitlist's coil availability, while Infor Visual carried it all along. The implementation note above assumed subordinate-part locations are rack labels "unrelated to plant location codes"; the live data contradicts that. See `defects/Closed-High-Setup-IgnoredLocationsDroppedTheJobsCoil.md`. The set remains the waitlist's inventory rule (`WaitlistInventoryService` + `InventoryLocationFiltering.Apply`).

...
- [ ] **Data Model: add canonical Category (umbrella) enum** with `Pickup`, `Deliver`, `Assist`, `Other`. (Ref: CSV col 1/6; Plan-Design-TypeCategoryActionRefactor) | **Persona: Backend Engineer**
- [ ] **Data Model: add canonical Item model** mirroring the CSV 18 rows (id `pickup-coil` … `other`, display, normalized, value type, source). (Ref: CSV rows; col 3/4/5/10) | **Persona: Backend Engineer**
...
*PREREQUISITE: confirm the Module_Setup field that supplies die location.*
...
- [ ] **Tech Lead: research how the product type (FG / WIP / Outside) is set through Infor Visual** and document the field/lookup to build. (Ref: CSV pickup-fg/ncm/wip/outside-service Notes; Plan-Design-TypeCategoryActionRefactor) | **Persona: Tech Lead**
- [ ] **Data Model: implement FG / WIP / Outside Item resolution** from the confirmed Infor Visual order lookup. (Ref: CSV rows; Plan-Design-TypeCategoryActionRefactor) | **Persona: Backend Engineer**
*PREREQUISITE: Infor Visual product-type research task above.*
**GATE: FG/WIP/Outside items resolve a real part/sequence (no hard-coded mock).**

...
- Added a step-by-step progress stepper to the app header for both the Work Center Setup and New Request flows, so you can always see which step you are on and what is left (with completed, current, and upcoming steps shown).
- Renamed the Dunnage Pair step to “Dunnage & Scrap” to match how the setup team talks about it.
- Added a “No Scrap” option to the scrap picker, so a job with no scrap can be recorded explicitly instead of leaving the field blank.
- Made the Dunnage & Scrap step require an explicit choice: you must pick a scrap type or select “No Scrap” before you can continue, with a short message shown when that choice is still missing.
- Added a confirmation prompt when you continue without selecting any dunnage, so a job is not accidentally saved with no dunnage.
...
- Before: The Work Center Setup and New Request flows only showed a text title in the header as you moved through the steps, and the Dunnage step could be continued past with the scrap picker left blank.
- After: The header now shows a visual progress stepper with completed, current, and upcoming steps, and the Dunnage & Scrap step makes you explicitly choose a scrap type or “No Scrap” — with a confirmation prompt before a no-dunnage job continues — so nothing is left to chance.
...
- The progress stepper shows exactly where you are in a multi-step job and what is left, so there is no guesswork about which step comes next.
- Requiring an explicit scrap decision (including “No Scrap”) prevents a job from being saved with a missing or accidental scrap value.
- Confirming before continuing with no dunnage prevents incomplete jobs from being saved by accident.
...
- You can always see where you are in the Setup or New Request flow and how many steps remain.
- The Dunnage & Scrap step now makes you explicitly record scrap (or “No Scrap”) and confirms before a no-dunnage job continues.

Verification:

- The project builds successfully.
- The test suite passes successfully, including the Setup and Waitlist workflow tests, the new New Request work center tests, and the new Dunnage & Scrap decision tests.
- The 25-work-center seed data, Last Updated tracking, the local/other section behavior, the New Request card + header behavior, the header progress stepper, and the Dunnage & Scrap decision flow were verified.
...
- Validate the header progress stepper and the Dunnage & Scrap decision flow against the real setup workflow.
...
- Added a shell header progress stepper (HeaderStep model, state enum, and template selector) that drives completed/current/upcoming step state from the Setup and New Request workflows, with localized step labels.
- Tightened the Dunnage & Scrap flow: added the “No Scrap” option, excluded the placeholder from the picker, gated Continue on an explicit scrap decision, added a no-dunnage confirmation dialog, and moved Add Dunnage into the section header.
- Centralized localization for the new stepper labels, scrap messages, and no-dunnage dialog in Resources.resw, and adjusted the startup log format.
- Added SetupDunnageTypeViewModelTests covering the scrap-decision gate and no-dunnage confirmation, plus a workflow-edgecase-inspection prompt and a XAML localization/resw checklist.
